...
- **自动起服务**：拉起 `dsh web`（默认 3080），窗口直接指向本地服务；只有通过桌面身份握手的同一实例才会复用，旧版/其他服务占用端口时自动切换随机回环端口
- **服务自愈**：每 5 秒用认证握手检查，服务进程意外退出（包括工具调用期间）会自动退避重启并刷新窗口，不再"断开后没反应"
...
- **dsh 自动就绪**：优先用应用自带的 dsh（装在用户目录，免管理员）；没有就自动安装固定版本 `@deepseek-ai/dsh@0.1.0-rc.7`，并校验 npm lockfile integrity；你手动装的（PATH 里有）也会直接用
- **本地服务认证**：应用为每次安装生成本地 desktop token，DSH API、静态页面和 WebSocket 都要求由 Electron 注入的认证请求头；认证 token 保存在应用用户目录，不写入仓库或 URL
- **应用里没有任何模型密钥**，也不碰你的密钥。模型 API Key 配置在你本机 dsh 的环境/配置里（`C:\Users\<你>\.dsh`），不进这个仓库、不被这个应用读取转发。服务只监听 127.0.0.1
- **桌面认证注意**：为了防止本机端口劫持，应用内置的 DSH Web 服务要求 Electron 注入 desktop token。直接用普通浏览器打开该服务会得到 401，这是预期行为；请通过 DSH Desktop 访问。
